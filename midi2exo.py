...
config['version']
modulepath = os.path.join(os.getcwd(), "pyaviutl")
module = importlib.import_module('pyaviutl.'+os.listdir(modulepath)[config['version']][:-3])
ExoVideo = module.ExoVideo
SceneSettings = module.SceneSettings
...
class Midi2ExoMain(QMainWindow):
    ...
    def about(self):
        ...
        msgBox.setInformativeText(
            "本程序可以利用 MIDI 文件生成 exo 文件，以减轻在 AviUtl 中音乐相关剪辑（如音MAD）的工作量。"
        )
    ...
    def open(self):
        self.file, _ = QFileDialog.getOpenFileName(
            self, "打开 MIDI 文件", filter="MIDI 文件 (*.mid)"
        )
        if self.file:
            self.handleMidi()
    ...
    def setDefSrcPath(self):
        path = QFileDialog.getExistingDirectory(
            self, "选择素材默认存放文件夹", self.defSrcPathLE.text()
        )
        if path != "":
            ...
            for msg in track:
                ...
                if msg.type == "set_tempo" and not targetTempo:
                    ...
                    if initial:
                        ...
                        self.channels.append(
                            Channel(
                                ...
                                [],
                                self.getPath(
                                    self.defSrcPathLE.text(),
                                    track.name,
                                    self.extLE.text(),
                                ),
                                self.defAlpha.checkState(),
                                self.defFlip.checkState(),
                            )
                        )
                        initial = False
                    if lastNote and lastNote.start == nowPosition:
                        continue
                    if lastNote:
                        self.channels[lastNote.objid].items.append(
                            ExoVideo(
                                start=tick2second(
                                    lastNote.start, self.midi.ticks_per_beat, nowTempo
                                ),
                                end=tick2second(
                                    nowPosition, self.midi.ticks_per_beat, nowTempo
                                ),
    ...
    def save(self):
        ...
        if self.anyNonExist():
            reply = QMessageBox.warning(
                self,
                "警告",
                "有轨道的素材文件不存在，这可能导致exo文件在导入时会不断报错，是否仍要导出？",
                QMessageBox.Yes | QMessageBox.No,
            )
            if reply == QMessageBox.No:
                return
        path, _ = QFileDialog.getSaveFileName(
            self, "导出 EXO 文件", filter="EXO 文件 (*.exo)"
        )
        if not path:
            return
        exo = {}
        ...
        exo["exedit"]["length"] = ceil(
            self.midi.length * exo["exedit"]["rate"]
        )  # !Important: Length must be calculated
        nowObj = 0
        for ch in self.channels:
            ...
            for v in ch.items:
                ...
                exo[nowObj]["start"], exo[nowObj]["end"] = 1 + ceil(
                    v["start"] * exo["exedit"]["rate"]
                ), ceil(v["end"] * exo["exedit"]["rate"])
                nowObj += 1
            if nowObj != 0:
    ...
    def refresh(self):
        for i in self.channels:
            if i.auto:
                i.path = self.getPath(
                    self.defSrcPathLE.text(), i.name, self.extLE.text()
                )
