...
class DropFileHandler(QObject):
    def eventFilter(self, watched, event):
        if event.type() == QEvent.DragEnter:
            ...
            if mime.hasUrls():
                watched.setText(
                    os.path.normpath(event.mimeData().urls()[0].toLocalFile())
                )
                return True
...
class Midi2ExoMain(QMainWindow):
    ...
    def save(self):
        ...
        for ch in self.channels:
            ...
            for v in ch.items:
                ...
                exo[nowObj]["start"], exo[nowObj]["end"] = 1 + math.ceil(
                    v["start"] * exo["exedit"]["rate"]
                ), math.ceil(v["end"] * exo["exedit"]["rate"])
                nowObj += 1
            if nowObj != 0:
    ...
    def render(self):
        ...
        self.extLE.setValidator(
            PyQt5.Qt.QRegularExpressionValidator(
                PyQt5.Qt.QRegularExpression("^[a-zA-Z0-9]*$")
            )
        )
